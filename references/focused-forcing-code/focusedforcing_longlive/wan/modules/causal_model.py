...
from wan.modules.focusedforcing.causal_rope_apply import causal_rope_apply_triton
from wan.modules.focusedforcing.focusedforcing import attention_with_selected_frames

from wan.modules.focusedforcing.cuda_ext import (
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if local_attn_size != -1 and (current_end > kv_cache["global_end_index"]) and (num_new_tokens + kv_cache["local_end_index"] > kv_cache_size):
                ...
                # print(f"clone kv_cache: {(t1 - t0)* 1000:.3f} ms")
            ...
            if sink_tokens > 0:
                ...
                if chunk_index >= 6:
                    ...
                    # torch.cuda.synchronize()
                    # t1 = time.perf_counter()
                    # print(f"compute attention_selected: {(t1 - t0)* 1000:.3f} ms")
                    ...
                    # torch.cuda.synchronize()
                    # t1 = time.perf_counter()
                    # print(f"compute attention: {(t1 - t0)* 1000:.3f} ms")
            else:
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_result = self.self_attn(
            ...
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, cache_start, sink_recache_after_switch, meta=meta)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_inference(
        ...
        for block_index, block in enumerate(self.blocks):
            meta.update({"block_index": block_index})
