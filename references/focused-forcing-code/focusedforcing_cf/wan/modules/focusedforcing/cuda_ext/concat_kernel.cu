...
void concat_cuda(
    ...
    TORCH_CHECK(a.is_cuda() && b.is_cuda() && c.is_cuda() && out.is_cuda(),
                "all tensors must be CUDA");
    TORCH_CHECK(a.is_contiguous() && b.is_contiguous() && c.is_contiguous() && out.is_contiguous(),
                "all tensors must be contiguous");

    TORCH_CHECK(a.dim() == 4 && b.dim() == 4 && c.dim() == 4 && out.dim() == 4,
                "all tensors must be [1, L, 12, 128]");
    TORCH_CHECK(a.size(0) == B_FIXED && b.size(0) == B_FIXED &&
                c.size(0) == B_FIXED && out.size(0) == B_FIXED,
                "batch must be 1");
    TORCH_CHECK(a.size(2) == H_FIXED && b.size(2) == H_FIXED &&
                c.size(2) == H_FIXED && out.size(2) == H_FIXED,
                "head dim must be 12");
    TORCH_CHECK(a.size(3) == D_FIXED && b.size(3) == D_FIXED &&
                c.size(3) == D_FIXED && out.size(3) == D_FIXED,
    ...
    const int Lc = static_cast<int>(c.size(1));
    const int Lo = static_cast<int>(out.size(1));

    TORCH_CHECK(Lo == La + Lb + Lc,
                "out.size(1) must equal La + Lb + Lc");
    ...
    switch (a.scalar_type()) {
        case torch::kFloat32:
            concat3_fixed_b1_kernel<float><<<blocks, THREADS, 0, stream>>>(
                a.data_ptr<float>(),
                b.data_ptr<float>(),
                c.data_ptr<float>(),
                out.data_ptr<float>(),
            ...
            break;
        case torch::kFloat16:
            concat3_fixed_b1_kernel<c10::Half><<<blocks, THREADS, 0, stream>>>(
                a.data_ptr<c10::Half>(),
                b.data_ptr<c10::Half>(),
                c.data_ptr<c10::Half>(),
        ...
        case torch::kBFloat16:
            concat3_fixed_b1_kernel<c10::BFloat16><<<blocks, THREADS, 0, stream>>>(
