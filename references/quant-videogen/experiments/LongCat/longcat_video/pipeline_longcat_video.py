...
class LongCatVideoPipeline:
    ...
    def _get_t5_prompt_embeds(
        ...
        prompt_embeds = prompt_embeds.view(
            batch_size * num_videos_per_prompt, 1, seq_len, -1
        )
    ...
    def check_inputs(
        self, prompt, negative_prompt, height, width, scale_factor_spatial
    ):
    ...
    def generate_t2v(
        ...
        if context_parallel_util.get_cp_rank() == 0:
            ...
            prompt_embeds = torch.zeros(
                [batch_size, 1, max_sequence_length, caption_channels],
                dtype=dit_dtype,
                device=device,
            )
        ...
        if not output_type == "latent":
            ...
            output_video = self.video_processor.postprocess_video(
                output_video, output_type=output_type
            )
        else:
    ...
    def generate_vc(
        ...
        # 1. Check inputs. Raise error if not correct
        assert not (
            use_distill and enhance_hf
        ), "use_distill and enhance_hf cannot both be True"
        scale_factor_spatial = self.vae_scale_factor_spatial * 2
        if self.dit.cp_split_hw is not None:
            scale_factor_spatial *= max(self.dit.cp_split_hw)
        height, width = self.get_condition_shape(
