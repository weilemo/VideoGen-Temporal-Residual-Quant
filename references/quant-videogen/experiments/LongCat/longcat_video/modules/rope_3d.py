...
from quant_videogen.kernels.triton.rope import rope_forward
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def precompute_freqs_cis_3d(self, grid_size):
        ...
        if self.cp_split_hw[0] * self.cp_split_hw[1] > 1:
            with torch.no_grad():
                freqs = rearrange(
                    freqs, "(T H W) D -> T H W D", T=num_frames, H=height, W=width
                )
                freqs = context_parallel_util.split_cp_2d(
                    freqs, seq_dim_hw=(1, 2), split_hw=self.cp_split_hw
                )
                freqs = rearrange(freqs, "T H W D -> (T H W) D")
