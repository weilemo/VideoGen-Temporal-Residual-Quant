...
class Attention(nn.Module):
    ...
    @ulysses_wrapper
    @time_logging_decorator("Process Attention", logging_level=3)
    def _process_attn(self, q, k, v, shape, timestep_int=None):
        """
        function wrapper to do attention with q, k, v
        """

        # if timestep_int is not None:
        #     if timestep_int % 4 == 0 and self.layer_idx % 4 == 0:
        #         with torch.no_grad():
        #             segment_idx = os.environ["SEGMENT_IDX"]
        #             file_name = f"assets/debug_tensor/long_480_fullkv/segment_idx_{segment_idx}/timestep_{timestep_int}_layer_{self.layer_idx}.pt"
        #             os.makedirs(os.path.dirname(file_name), exist_ok=True)

        #             print(f"Saving debug tensor to {file_name}")
        #             torch.save(
        #                 {
        #                     "q": q.detach().cpu(),
        #                     "k": k.detach().cpu(),
        ...
        #             print(
        #                 f"Memory usage (GB): {torch.cuda.memory_allocated() / 1024 ** 3}"
        #             )
        ...
        if (
            self.enable_bsa and shape[0] > 1
        ):  # bsa will not be used in image training / sampling
    ...
    def forward(
        ...
        """ """
        with time_logging_decorator("QKV Linear", logging_level=3):
            B, N, C = x.shape
            qkv = self.qkv(x)
        ...
        with time_logging_decorator("Rope 3D", logging_level=3):
            ...
            #     with torch.no_grad():
            #         segment_idx = os.environ["SEGMENT_IDX"]
            #         file_name = f"assets/debug_tensor/long_480p_fullkv/segment_idx_{segment_idx}/_pre_rope_layer_{self.layer_idx}.pt"
            #         os.makedirs(os.path.dirname(file_name), exist_ok=True)
            ...
            #     with torch.no_grad():
            #         segment_idx = os.environ["SEGMENT_IDX"]
            #         file_name = f"assets/debug_tensor/long_480p_fullkv/segment_idx_{segment_idx}/_post_rope_layer_{self.layer_idx}.pt"
            #         os.makedirs(os.path.dirname(file_name), exist_ok=True)
        ...
        if num_cond_latents is not None and num_cond_latents > 0:
            with time_logging_decorator("Cond Mode 1", logging_level=3):
            ...
            with time_logging_decorator("Cond Mode 2", logging_level=3):
            ...
            x = self._process_attn(q, k, v, shape, timestep_int=timestep_int)
