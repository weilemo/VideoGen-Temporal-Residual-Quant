...
def all_to_all(tensor, scatter_idx, gather_idx, group=None, gather=True):
    ...
    def chunk_tensor(tensor, scatter_idx):
        ...
        if t_shape[scatter_idx] % world_size != 0:
            raise ValueError(
                f"Dimension {scatter_idx} must be divisible by world size {world_size}"
            )
        ...
        tensor = tensor.permute(
            scatter_idx, *[i for i in range(len(new_shape)) if i != scatter_idx]
        ).contiguous()
        return tensor
    ...
    def reorder_tensor(tensor, gather_idx):
        ...
        tensor = tensor.permute(
            *permute_idx
        ).contiguous()  # permute(1,2,0,3) W B CH CS D -> B CH W CS D
...
def ulysses_a2a_in(query, key, value):
    ...
    # [B, H, S/N, D] -> [B, H/N, S, D]
    query = all_to_all(
        query, scatter_idx=1, gather_idx=2, group=context_parallel_util.get_cp_group()
    )
    key = all_to_all(
        key, scatter_idx=1, gather_idx=2, group=context_parallel_util.get_cp_group()
    )
    value = all_to_all(
        value, scatter_idx=1, gather_idx=2, group=context_parallel_util.get_cp_group()
    )
    return query, key, value
...
def ulysses_a2a_out(output):
    ...
    # [B, H/N, S, D] -> [B, H, S/N, D]
    output = all_to_all(
        output, scatter_idx=2, gather_idx=1, group=context_parallel_util.get_cp_group()
    )
    return output


def ulysses_wrapper(func):
    def wrapper(self, query, key, value, shape, timestep_int=None):
        ...
        output = func(self, query, key, value, shape, timestep_int=timestep_int)
