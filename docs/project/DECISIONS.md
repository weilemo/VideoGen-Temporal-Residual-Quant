...
- 决策：保留 `temporalresidualkvquant/` 作为方法工作区；基线适配统一放入
  `integrations/`，共享协议放入 `experiments/`，上游快照放入 `references/`。
- 原因：此前方法代码、上游源码、运行脚本和项目记录混在多个目录，难以判断所有权，
  也容易在基线源码中形成第二份 TRQ 实现。
- 影响：除暂时保留的 Self-Forcing vendored backend 外，上游基线默认在远端安装，
  本仓库维护适配器和可重放补丁；本地不下载权重、不跑 GPU，代码完成后同步远端验证。

## D-2026-07-24-13 项目记录归档到 `docs/`

- 决策：当前状态、交接、长期事实和决策进入 `docs/project/`，单次执行记录进入
  `docs/history/`。
- 原因：仓库根目录只保留代码入口和全局协作约定，减少新协作者的入口噪声。
- 影响：新的 agent 先读 `AGENTS.md`、`docs/project/STATUS.md` 和
  `docs/project/HANDOFF.md`；旧日志中的历史绝对路径不改写。
