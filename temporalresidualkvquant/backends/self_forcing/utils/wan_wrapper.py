...
def _self_forcing_ckpt_path(*parts: str) -> str:
    ckpt_root = os.getenv("SELF_FORCING_CKPT_ROOT", "ckpts/Self-Forcing")
    return os.path.join(ckpt_root, *parts)
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(_self_forcing_ckpt_path("Wan2.1-T2V-1.3B", "models_t5_umt5-xxl-enc-bf16.pth"),
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=_self_forcing_ckpt_path("Wan2.1-T2V-1.3B", "google", "umt5-xxl"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=_self_forcing_ckpt_path("Wan2.1-T2V-1.3B", "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                _self_forcing_ckpt_path(model_name), local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained(_self_forcing_ckpt_path(model_name))
        self.model.eval()
