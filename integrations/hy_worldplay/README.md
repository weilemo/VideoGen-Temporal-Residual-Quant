...
upstream implementation in `references/quant-videogen` and records only the
...
```bash
python integrations/hy_worldplay/runner.py --matrix --dry-run
python integrations/hy_worldplay/runner.py --variant bf16 --action canonical
python integrations/hy_worldplay/runner.py --variant trq_int2 --action turn_left
```
...
```bash
HY_WORLDPLAY_SOURCE=/path/to/Quant-VideoGen \
  python integrations/hy_worldplay/runner.py --matrix --dry-run
```
