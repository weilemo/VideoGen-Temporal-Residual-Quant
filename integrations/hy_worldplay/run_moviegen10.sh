...
qvg_root="${HY_WORLDPLAY_SOURCE:-${repo_root}/references/quant-videogen}"
export PYTHONPATH="${repo_root}/temporalresidualkvquant/src:${qvg_root}:${qvg_root}/experiments/HY-WorldPlay${PYTHONPATH:+:${PYTHONPATH}}"
...
for variant in "${variants[@]}"; do
  ...
  python "${repo_root}/integrations/hy_worldplay/runner.py" \
    --variant "${variant}" \
    --action canonical \
