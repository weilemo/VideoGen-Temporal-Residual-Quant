...
```bash
bash experiments/world_model_quant/download_longcat.sh \
  "$HOME/storage/models/LongCat-Video" forcing/longcatvideo

bash experiments/world_model_quant/run_longcat_moviegen10.sh bf16 prefix
for mode in bf16 trq_int4 trq_int2 naive_int4 naive_int2; do
  bash experiments/world_model_quant/run_longcat_moviegen10.sh "$mode" continuation
done
