...
from trq import offload_kv_cache_layer, onload_kv_cache_layer
...
def causal_rope_apply_long_input(x, grid_sizes, freqs):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        num_chunks = input_seq_len // seq_len
        assert input_seq_len % seq_len == 0, "Input sequence length is not divisible by sequence length"
        out_chunks = []
        for chunk_idx in range(num_chunks):
            chunk_start = chunk_idx * seq_len

            # precompute multipliers
            x_chunk = torch.view_as_complex(
                x[i, chunk_start:chunk_start + seq_len].to(torch.float64).reshape(
                    seq_len, n, -1, 2
                )
            )
            ...
            # apply rotary embedding
            x_chunk = torch.view_as_real(x_chunk * freqs_i).flatten(2)
            out_chunks.append(x_chunk)

        output.append(torch.cat(out_chunks, dim=0) if len(out_chunks) > 0 else x[i])

    result = output[0].unsqueeze(0) if len(output) == 1 else torch.stack(output)
...
class CausalWanSelfAttention(nn.Module):
    ...
    def attn_kv_cache_is_none(self, q, k, v, block_mask, grid_sizes, freqs, seq_lens, s):
    ...
    def attn_kv_cache(self, kv_cache, current_start, q, k, v, block_mask, grid_sizes, freqs, seq_lens, s):
        ...
        if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
            ...
            kv_cache["k"][:, local_start_index:local_end_index] = roped_key
            kv_cache["v"][:, local_start_index:local_end_index] = v
            if "k_no_rope" in kv_cache:
                kv_cache["k_no_rope"][:, local_start_index:local_end_index] = k
    ...
    def attn_kv_cache_prerope(self, kv_cache, current_start, q, k, v, block_mask, grid_sizes, freqs, seq_lens, s):
        """
        To facilitate this, KV cache should always stay with in the kv cache size.
        """
        frame_seqlen = math.prod(grid_sizes[0][1:]).item()
        current_start_frame = current_start // frame_seqlen
        roped_query = causal_rope_apply(
        ...
        # If we are using local attention and the current KV cache size is larger than the local attention size, we need to truncate the KV cache
        kv_cache_size = kv_cache["k"].kv_cache_size
        num_new_tokens = roped_query.shape[1]
        if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                num_new_tokens + kv_cache["local_end_index"].item() > kv_cache_size):
        ...
        assert local_end_index <= self.max_attention_size, f"KV cache size exceeds the max attention size: {local_end_index} <= {self.max_attention_size}"
    ...
    def forward(
        ...
        if kv_cache is None:
            x = self.attn_kv_cache_is_none(q, k, v, block_mask, grid_sizes, freqs, seq_lens, s)
        else:
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _forward_inference(
        ...
        do_offload = getattr(self, "kv_cache_cpu_offload", False)

        for block_index, block in enumerate(self.blocks):
            if do_offload:
                onload_kv_cache_layer(kv_cache[block_index], device)
            ...
            if do_offload:
                offload_kv_cache_layer(kv_cache[block_index])
